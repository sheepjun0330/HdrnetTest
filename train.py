...
def train(params=None):
    ...
    for e in range(params['epochs']):
        ...
        for i, (low, full) in enumerate(train_loader):
            ...
            mse_loss = mseloss(full,res).item()
            clip_loss = cliploss.clip_directional_loss(full, res, text_direction).item()
            patch_loss = cliploss.patch_loss(full, res, text_direction).item()
            #tv_loss = cliploss.tv_loss(res).item()

            total_loss = lam1*mse_loss + lam2*clip_loss + lam3*patch_loss
            total_loss.backward()
            train_losses["mse_loss"].append(mse_loss)
            train_losses["clip_loss"].append(clip_loss)
            train_losses["patch_loss"].append(patch_loss)
        ...
        with torch.no_grad():
            for i, (low, full) in enumerate(valid_loader):
                ...
                #tv_loss = cliploss.tv_loss(res).item()
                total_loss = lam1*mse_loss + lam2*clip_loss + lam3*patch_loss 
                valid_losses["mse_loss"].append(mse_loss)
                valid_losses["clip_loss"].append(clip_loss)
                valid_losses["patch_loss"].append(patch_loss)
