...
def train(params=None):
    ...
    train_losses = {"mse_loss":[], "clip_loss":[], "total_loss":[]}
    valid_losses = {"mse_loss":[], "clip_loss":[], "total_loss":[]}
    count = 0
    count2 = 0
    for e in range(params['epochs']):
        ...
        for i, (low, full) in enumerate(train_loader):
            ...
            total_loss = (1-alpha)*mse_loss + alpha*clip_loss
            total_loss.backward()
            ...
            if (count+1) % params['log_interval'] == 0:
                ...
                wandb.log({"Training MSE Loss": train_mse_loss, "Training CLIP Loss": train_clip_loss, "Training Total Loss": train_total_loss})
                train_losses = {"mse_loss":[], "clip_loss":[], "total_loss":[]}
        ...
        with torch.no_grad():
            for i, (low, full) in enumerate(valid_loader):
                ...
                if (count2+1) % params['log_interval'] == 0:
                    ...
                    wandb.log({"Validation MSE Loss": valid_mse_loss, "Validation CLIP Loss": valid_clip_loss, "Validation Total Loss": valid_total_loss})
                    valid_losses = {"mse_loss":[], "clip_loss":[], "total_loss":[]}
                count2 += 1
